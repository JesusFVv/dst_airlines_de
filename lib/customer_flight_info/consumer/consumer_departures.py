...
CHANNEL_NAME=os.environ['CUSTOMERS_FLIGHT_INFO_DEPARTURES_CHANNEL']
...
def main():
    ...
    def callback(ch, method, properties, body):
        ...
        try:
            run_customer_flight_information_pipeline(CUSTOMER_FLIGHT_INFO_DEPARTURE_AIRPORT_ENDPOINT, message['date'], message['airport_iata'], CUSTOMER_FLIGHT_INFO_TABLE_NAME)
...
if __name__ == "__main__":
    logFormatter = logging.Formatter("%(asctime)s [%(levelname)-5.5s] %(name)s: DEPARTURES %(message)s")
    fileHandler = logging.FileHandler(f"{LOG_FILE_PATH}/consumer_departures.log")
