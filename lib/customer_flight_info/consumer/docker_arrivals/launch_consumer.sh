...
#   bash lib/customer_flight_info/consumer/docker_arrivals/launch_consumer.sh
set -a
source .env
set +a
CONTAINER_NAME=customer_flight_information_arrivals_consumer
...
docker container run -d --name $CONTAINER_NAME \
    ...
    --restart always \
    dst_customer_flight_information_arrivals_consumer
