...
CUSTOMER_FLIGHT_INFO_STAGGING_TABLE_NAME = os.environ['CUSTOMER_FLIGHT_INFO_STAGGING_TABLE_NAME']
CUSTOMER_FLIGHT_INFO_L2_TABLE_NAME = os.environ['CUSTOMER_FLIGHT_INFO_L2_TABLE_NAME']
REFDATA_AIRPORTS_L2_TABLE_NAME = os.environ['REFDATA_AIRPORTS_L2_TABLE_NAME']
...
def build_flat_data(
    ...
    for tuple_data in all_data:
        ...
        if formatted_dict:
            yield (data, formatted_dict)


def ingest_data(
    db_config_filepath: PosixPath | None,
    sql_table_name_cooked: str,
    stagging_table_name: str,
    gen: Generator[tuple[dict], None, None],
) -> None:
    ...
    cnt = 1
    for original_data, cooked_data in gen:
        try:
            logger.info(cooked_data)
            utils.insert_data_into_db(cur, sql_table_name_cooked, cooked_data)
            delete_row_in_stagging(cur, stagging_table_name, original_data)
            conn.commit()
            # conn.rollback()
            if cnt%1000==0:
...
def delete_row_in_stagging(cur: psycopg2.extensions.cursor, db_table_name: str, data: dict) -> None:
    """Delete the ingested row from the stagging table
    ...
    """
    query = f"delete from {db_table_name} where data = %s"
...
def main() -> None:
    ...
    ingest_data(db_config_filepath, sql_table_name_cooked, raw_table_name, gen)
    logger.info("COOKED LOADING COMPLETED !")


if __name__ == "__main__":
    logFormatter = logging.Formatter("%(asctime)s [%(levelname)-5.5s]  %(message)s")
    fileHandler = logging.FileHandler(f"{LOG_FILE_PATH}/l2_incremental_loading.log")
