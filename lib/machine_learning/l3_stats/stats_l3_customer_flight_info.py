...
def ingest_data(
    ...
    l3_stats_airports_table_name: str,
    truncate_l3_stats_airports_query: str,
    ...
    Args:
        ...
        l3_stats_airports_table_name (str): SQL table name where raw cooked are stored
        truncate_l3_stats_airports_query (str): SQL query to truncate cooked table
    ...
    cur.execute(truncate_l3_stats_airports_query)
    conn.commit()
    ...
    for cooked_data in gen:
        try:
            utils.insert_data_into_db(cur, l3_stats_airports_table_name, cooked_data)
            conn.commit()
            if cnt%1000==0:
...
if __name__ == "__main__":
    ...
    l2_ops_custo_flights_table = "l2.operations_customer_flight_info"
    l2_full_flights_query = f"SELECT * FROM {l2_ops_custo_flights_table} WHERE departure_actual_datetime IS NOT NULL AND arrival_actual_datetime IS NOT NULL"

    l3_airports_table_name = "l3.view_airports"
    l3_airports_data_query = f"SELECT * FROM {l3_airports_table_name}"
    
    l3_birds_table_name = "l3.bird_strikes"
    l3_birds_data_query = f"SELECT * FROM {l3_birds_table_name}"

    l3_stats_airports_table_name = "l3.stats_airports"
    truncate_l3_stats_airports_query = f"TRUNCATE TABLE {l3_stats_airports_table_name}"

    ########################
    # Read data from database l2.operations_customer_flight_info table
    full_flights_data = utils.read_dataframe_from_db(
        db_config_filepath, l2_full_flights_query
    ) 
    # Read data from database l3.view_airports table
    df_airports_data = utils.read_dataframe_from_db(
        db_config_filepath, l3_airports_data_query
    )
    
    # Read data from database l3.view_airports table
    df_birds = utils.read_dataframe_from_db(
        db_config_filepath, l3_birds_data_query
    )
    ...
    gen = build_flat_data(
        full_flights_data, df_airports_data
    )  # Generator object
    # Ingest data into cooked table
    ingest_data(db_config_filepath, l3_stats_airports_table_name, truncate_l3_stats_airports_query, gen)
