...
# Set some variables
readonly container_name="python_load_cooked_data"
readonly image_name_python="python:3.11-alpine3.20"
readonly image_name_app="python_load_customer_flight_info_cooked:latest"
...
rm -rf ${tmp_build_context}
# Run Docker
docker run --network ${network_name} --name ${container_name} ${image_name_app}
